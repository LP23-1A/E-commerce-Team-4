...
const page = () => {
  ...
  const putProduct = async () => {
    const productId = JSON.parse(localStorage.getItem("putProduct") as string)
    try {
        const res = await axios.put(`http://localhost:8000/products/${productId}`, { ...formDataRef.current })
        console.log(res);
        router.push('/Product')
    } catch (error) {
        console.log(error);
                ...
                <div className="flex flex-col gap-5 w-1/2">
                  <div className="w-full bg-white rounded-xl p-6 gap-2 flex flex-col">
                    <div className="flex flex-col gap-1">
                      ...
                      <input type="text" className="border w-full p-3 rounded-lg" placeholder={name} onChange={(e) => handleRef("productName", e.target.value)}/>
                    </div>
                    <div className="flex flex-col gap-1">
                      <h1 className="font-semibold">Нэмэлт мэдээлэл</h1>
                      <input type="text" className="border w-full p-3 rounded-lg h-fit" placeholder={description} onChange={(e) => handleRef('description', e.target.value)}/>
                    </div>
                    <div className="flex flex-col gap-1">
                      <h1 className="font-semibold">Барааны код</h1>
                      <input type="text" className="border w-full p-3 rounded-lg" placeholder={productCode} onChange={(e) => handleRef('productCode', e.target.value)}/>
                    </div>
                  </div>
                  ...
                  <div className="w-full h-[163px] bg-white rounded-xl p-6 gap-2 flex justify-between">
                    <div className="flex flex-col gap-1">
                      ...
                      <input type="text" placeholder={price} className="border p-2 rounded-lg w-[230px]" onChange={(e) => handleRef('price', e.target.value)}/>
                    </div>
                    <div className="flex flex-col gap-1">
                      <h1 className="font-semibold">Үлдэгдэл тоо ширхэг</h1>
                      <input type="text" placeholder={residual} className="border p-2 rounded-lg w-[230px]" onChange={(e) => handleRef('residual', e.target.value)}/>
                    </div>
                  </div>
                </div>
                <div className="flex flex-col gap-5 w-1/2">
                  <div className="w-full bg-white rounded-xl p-6 gap-2 flex flex-col">
                    <div className="flex flex-col gap-1">
                      ...
                      <input type="text" className="border w-full p-3 rounded-lg h-16" placeholder={mainCate} onChange={(e) => handleRef('mainCate', e.target.value)}/>
                    </div>
                    <div className="flex flex-col gap-1">
                      <h1 className="font-semibold text-lg">Дэд ангилал</h1>
                      <input type="text" className="border w-full p-3 rounded-lg h-16" placeholder={subCate} onChange={(e) => handleRef('subCate', e.target.value)}/>
                    </div>
                  </div>
                  ...
                  <div className="w-full bg-white rounded-xl p-6 gap-2 flex flex-col">
                    ...
                    <input type="text" className="border w-full p-3 rounded-lg h-16" placeholder={tag} onChange={(e) => handleRef('categoryId', e.target.value)}/>
